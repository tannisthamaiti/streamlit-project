...
def make_api_request(token):
    ...
    api_endpoint = 'https://greenscanbackend-production.up.railway.app/api/token/is-valid'   
    
    response = requests.post(api_endpoint, data={'token': token})
    
    if response.status_code == 200:
        response_data = response.json()
        return (response_data['data']['isValid'])
...
if 'token' in query_params: 
    ...
    show_content =  make_api_request(token)
...
if not show_content:
    st.markdown("<p style='font-size: 35px; font-weight: light; font-family: Inter; color: black;text-align:center;'>You are not authorized!</p>", unsafe_allow_html=True)
    ...
    st.markdown("<p style='font-size: 35px; font-weight: light; font-family: Inter; color: black'>Interactive solar energy plotter</p>", unsafe_allow_html=True)
    ...
    with col1:
        st.markdown("<p style='font-size: 19px; color: black; font-weight: light; font-family: Inter;'>Parameters Selection: Choose the parameters</p>", unsafe_allow_html=True)

        col4, col5, col6 = st.columns([1, 1, 1])
        with col4:
            # st.markdown("<style>.checkbox-label { color: black !important; }</style>", unsafe_allow_html=True)
        ...
        with col6:
            st.markdown("<p style='font-size:19px; color:black; font-weight:light; font-family: Inter;'>Date selection</p>", unsafe_allow_html=True)
    ...
    with col2:
        ...
        ax1.axis('equal')
        plt.title('Pie Chart showing anomaly percentage', color='white')
